...
class assembly_parser(object):
    ...
    def second_pass(self, lines):
        ...
        for line in lines:
            ...
            if ':' in line:
                label = line[0:line.find(':')]
                line = line[line.find(':') + 1:].strip()
                self.output_array.append( '//' + hex(int(self.symbol_table[label])) + ' <' + label + '>:' )
    ...
    def parse_instruction(self, instruction, raw_args):
        ...
        for arg in args:
            if '(' in arg:
                ...
                #IF REGISTER IS IN PARENTHESES, IT NEEDS SYMBOL FOR REGISTER, NOT NUMBER. THIS FIXED IT.
                # Parse offset from known syntax
                offset   = hex(int(arg[0:arg.find('(')]))
                register = re.search('\((.*)\)', arg)
                #print(self.register_table.keys())
                # Location in memory is offset from memory location
                #location = self.register_table[register.group(1)]
                #register = location
                location = register.group(1)
                if location in self.register_table.keys():
                    #print("1")
                # Replace symbol with value in table
                    register = int(self.register_table[location])
    ...
    def store_bit_string(self, bit_string, instruction, arguments):
        ...
        if self.current_location % 4 == 0:
			...
			cur_loc = hex(self.current_location)
        ...
        if self.current_location %4 == 0:
            ...
            self.output_array[-1] += ' // ' + cur_loc + ': ' + instruction.ljust(5) + ', '.join(arguments)
    ...
    def print_memory_map(self):
        ...
        print '// BYU ECEN 323 Assembler version 1.0'
        print '// The memory map in HEX:'
        for output in self.output_array:
            print output
        print '\n'
        print "// The label list is: " + str(self.symbol_table)
